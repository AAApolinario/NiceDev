...
window.addEventListener("load", function () { //document.ready(function() {
	// Recupera Variável da Query da URL
	const urlParams = new URLSearchParams(window.location.search);
	var templateParam = urlParams.get("negocio");
	init(templateParam);
	//Tela inicial
	read_login();
	Initialize();
...
var ChatAtivo = false;
var PauseAlreadyRun = false

var transferValidate = false;
var parentContactId = "0";
var Addressbooktransfer = false;
var GetSkillsByAgent;
var HasOutboundSkills = false;
...
api.onBettaDriverOpen = function (event) {
	...
	console.log('api.onBettaDriverOpen>>');
	//read_login();
	//ConfigCTI();
	inttentativas = 0;
}
//v2.6
var inttentativas = 0; 		//contador de tentativas de reconexão
var intmaxtentativas = 10; 	//máximo de tentativas de reconexão
var intintervalms = 5000; 	//intervalo em milissegundos para a tentativa de reconexão
api.onBettaDriverClose = async function(event){
	inttentativas++;
	console.log('api.onBettaDriverClose >>', inttentativas);
	if (inttentativas<intmaxtentativas)
	{
		intervalID = await setTimeout(reconnectWS, intintervalms);
	}
} 

//v.2.6
api.onBettaDriverMessage = function (event) {
	loadingOverlay.style.display = 'none';
	console.log('api.onBettaDriverMessage >>', event.data);
}
//v2.6
async function reconnectWS(intervalID) {
...
api.onBettaDriverError = function (event) {
	var strtype = event;
	console.log('api.onBettaDriverError >>');
...
if (event.ANI == "Restricted" || event.ANI == "1" ) {
		strAniInCall.innerText = event.DNIS
	} else {
...
api.OnCallDisconnected = function (event) {
	...
	document.getElementById('dtmf-sequence').textContent = '';
...
api.OnGetAgentState = function (event) {
	...
	if (strloggedOnState == "True") {
		...
		var agentState = event.agentState;
		var reasonCode = event.reasonCode;
		var divWelcome = $("#div-welcome");

		if (agentState === "agentReady") {
			divWelcome.removeClass("unavailable");
			$("#agent-status").text("Disponível");
		} else if (agentState === "agentNotReady" && reasonCode) {
			divWelcome.addClass("unavailable");
			$("#agent-status").text("Em pausa: " + reasonCode);
		} else if (agentState === "agentNotReady") {
			divWelcome.addClass("unavailable");
			$("#agent-status").text("Indisponível");
		} else {
			divWelcome.addClass("unavailable");
			$("#agent-status").text("Estado: " + agentState);
		}
...
api.OnGetSkills = function (event) {
	console.log('OnGetSkills', event);
	strMatrizTransf = event.Skills;
	GetSkillsByAgent = event.byagent;
	const OptionChat = document.querySelector('#select-transfer-chat');
	//Get Skills Assigned to an agent;
	if (GetSkillsByAgent == "byagent") {
		for (i in strMatrizTransf) {
			if (strMatrizTransf[i].isOutbound == "true" && strMatrizTransf[i].mediaTypeName == "Phone Call") {
				$("#OutboundSkillsList").append('<option value="' + strMatrizTransf[i].skillId + '">' + strMatrizTransf[i].skillName + '</option>');
				HasOutboundSkills = true
			}
		}
	}
	//Get Skills;
		...
		for (i in strMatrizTransf) {
			if (strMatrizTransf[i].mediaTypeName == "Phone Call" && strMatrizTransf[i].isOutbound == "false") {
				$("#select-transfer-itau").append('<option value="' + strMatrizTransf[i].skillId + '">' + strMatrizTransf[i].skillName + '</option>');
			}
			if (strMatrizTransf[i].mediaTypeName == "Chat" && strMatrizTransf[i].isOutbound == "false") {
				var NewOption = new Option(strMatrizTransf[i].skillName, strMatrizTransf[i].skillId);
				OptionChat.appendChild(NewOption);
			}
		}
	}
	if (HasOutboundSkills == true) {
		api.getaddressbook();
	}
	$("#select-transfer-itau").val('');
}

api.OnGetTeamPauseReason = function (event) {
	console.log('OnGetTeamPauseReason', event.PauseReasons);
	var PauseReasonsAPI = event.PauseReasons;

	for (let a = 0; a < PauseReasonsAPI.length; a++) {
		//Create li element inside ul
		const ulID = document.getElementById('agentstateui-0stateList');
		const liCreate = document.createElement('li');
		liCreate.setAttribute('id', PauseReasonsAPI[a].id)
		ulID.appendChild(liCreate)

		//Create span element inside li
		let liID = document.getElementById(PauseReasonsAPI[a].id);
		const spanCreate = document.createElement('span');
		spanCreate.setAttribute('id', PauseReasonsAPI[a].id);
		spanCreate.setAttribute('onclick', "Pause(this)");
		spanCreate.innerHTML = `${PauseReasonsAPI[a].reason}`;
		liID.appendChild(spanCreate);
	}
}

api.OnGetAddressbook = function (event) {
	console.log('OnGetAddressbook', event);
	if (event.addressBooks.length >= 1) {
		strMatrizAddressbook = event.addressBooks;
		try{
			for (let i in strMatrizAddressbook) {
			let addressBookArray = strMatrizAddressbook[i].addressBookEntries;
			if(addressBookArray != null){
				if (addressBookArray.length > 1) {
					for (let index = 0; index < addressBookArray.length; index++) {
						InsideAddressBook = addressBookArray[index];
						$("#select-transfer-itau").append('<option class="AddressBook" value="' + InsideAddressBook.phone + '">' + InsideAddressBook.firstName + " " + InsideAddressBook.lastName + '</option>');
					}
				} else {
					strAddressBookArray = addressBookArray[0];
					$("#select-transfer-itau").append('<option value="' + strAddressBookArray.phone + '">' + strAddressBookArray.firstName + " " + strAddressBookArray.lastName + '</option>');
				}
			}
		}
		}catch(err){
...
async function Logout() {
	...
	$("#lbllogin").show();
	blnlogoutrequest = true;
	TurnUnavailable()
	api.logout();
	await validate_logout();
}
...
function Pause(ReasonPause) {
	strreason = ReasonPause.textContent
...
function MenuOutbound(Origem) {
	$(screenInFocus).hide();
	$("#SCREEN-OUTBOUND").show();
	screenInFocus = "#SCREEN-OUTBOUND"
}
...
function AgentLogged() {
	if ($("#main-panel").is(":hidden")) {
		...
		if (PauseAlreadyRun == false) {
			api.getskills();
			api.getteampausereason();
			api.getskills('byagent');
			PauseAlreadyRun = true;
		}
	}
}
...
function ValidateTransfer(FromIndex) {
	var select = document.getElementById('select-transfer-itau').value;
	var Skills_Outbound_List = document.getElementById("OutboundSkillsList")
	var chamada = 'option[value="' + select + '"]'
	var objectTransfer = document.querySelectorAll(chamada);
	var skilloutboundselected;
	objectTransfer = objectTransfer[0].classList

	//Verifica se o Objeto é um Skill
	if (objectTransfer == "" && FromIndex == 'true') {
		$('#OutboundSkillsListDiv').css("display", "none");
		transferValidate = true;
		Addressbooktransfer = false;
	}
	//Verifica se o Objeto é um AddressBook
	if (objectTransfer == "AddressBook") {
		if (Skills_Outbound_List.length > 1) {
			$('#OutboundSkillsListDiv').css("display", "grid");
			transferValidate = true;
			Addressbooktransfer = true;
		}
		if (Skills_Outbound_List.length == 1) {
			skilloutboundselected = Skills_Outbound_List.value;
			transferValidate = true;
			Addressbooktransfer = true;
		}
	}
	//Make Call
	if (transferValidate == true && Skills_Outbound_List.value != "" && FromIndex == 'false' && Addressbooktransfer == true) {
		colocarEmEspera();
		skilloutboundselected = Skills_Outbound_List.value;
		api.makecall(select, skilloutboundselected, parentContactId);
		$("#select-transfer-itau").val('');
		$('#OutboundSkillsListDiv').css("display", "none");
		$(screenInFocus).hide();
		$("#SCREEN-INCALL").show();
		screenInFocus = "#SCREEN-INCALL";
		$('#btn-transferVoice').show();
	}
	//Transferência Fria
	if (transferValidate == true && FromIndex == 'false' && Addressbooktransfer == false) {
		MatrizTranf(select);
	}
}
...
function UraAutenticadora() {
	p1 = "UraAutenticadora";
	api.signal(CallContactID, p1);
	console.Log("Chamando UraAuntenticadora " + CallContactID);
}

function UraPesquisa() {
	p1 = "Ura_Pesquisa";
	api.signal(CallContactID, p1);
	console.Log("Chamando UraPesquisa " + CallContactID);
}

function ClearPauseAndSkills() {
	HasOutboundSkills = false;
	var DeleteList = []
	DeleteList[0] = document.getElementById("agentstateui-0stateList");
	DeleteList[1] = document.getElementById("select-transfer-itau");
	DeleteList[2] = document.getElementById("OutboundSkillsList");
	DeleteList[3] = document.getElementById("select-transfer-chat");

	for (i = 0; i < DeleteList.length; i++) {
		try {
			while (DeleteList[i].firstChild) {
				DeleteList[i].removeChild(DeleteList[i].firstChild);
			}
		} catch (err) {
			console.log(err)
		}
...
function makeOutboundCall() {
	...
	if (number.length > 0) {
		if (callconnected == true) {
			api.consultationcall(number);
		} else {
			const SkillIdOutbound_List = document.getElementById("OutboundSkillsList");
			let skillId = null;

			if (SkillIdOutbound_List.options.length === 1) {
				// If there's only one skill, use it
				skillId = SkillIdOutbound_List.options[0].value;
			} else if (SkillIdOutbound_List.options.length > 1) {
				// If there are multiple skills, show a pop-up for the user to select a skill
				$("#SCREEN-OUTBOUND").hide();
				let skillOptions = Array.from(SkillIdOutbound_List.options).map(option => `<option value="${option.value}">${option.text}</option>`).join('');
				let skillPopup = document.createElement('div');
				skillPopup.id = 'skill-popup';
				skillPopup.innerHTML = `
					<div style="position: fixed; top: 25%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 20px; border: 1px solid #ccc; border-radius: 8px; z-index: 1000;">
						<h3>Selecione um Skill</h3>
						<select id="selected-skill">${skillOptions}</select>
						<br><br>
						<button id="confirm-skill">Confirmar</button>
						<button id="cancel-skill">Cancelar</button>
					</div>
				`;
				document.body.appendChild(skillPopup);

				document.getElementById('confirm-skill').addEventListener('click', () => {
					skillId = document.getElementById('selected-skill').value;
					if (!skillId) {
						alert("Por favor, selecione um skill antes de fazer a ligação.");
					} else {
						api.makecall(number, skillId, parentContactId);
						document.body.removeChild(skillPopup);
						$(screenInFocus).hide();
						$("#SCREEN-WELCOME").show();
						screenInFocus = "#SCREEN-WELCOME";
						clearNumber();
				...
				alert("Nenhum skill de saída disponível.");
				return;
			}
...
function UraTeclado() {
	$("#SCREEN-INCALL").hide();
	document.getElementById('SCREEN-DTMF').style.display = 'block';
	screenInFocus = "#SCREEN-DTMF"
}

function sendDTMF(digit) {
	const sequenceDisplay = document.getElementById('dtmf-sequence');
	sequenceDisplay.textContent += digit;

	// Envia o DTMF para a API
	api.ctisenddtmf(digit);
}
