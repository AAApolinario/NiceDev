...
function RecuperaSession(){
	...
	BoxFooter = document.querySelector(`.foot-${result}`);
		if(result === "session-id-2" && ChatRoom == null && islogged == true){
			MainChatContent.style.display = "none"
			if (screenInFocus == "#SCREEN-INCALL") {
				$("#MAIN").show();
				$(screenInFocus).hide();
				$("#SCREEN-WELCOME").show();
		...
		if(result != "session-id-2" && callconnected == true){
			MainChatContent.style.display = "none"
			$("#MAIN").show();
			if(screenInFocus == "#SCREEN-WELCOME"){
				$(screenInFocus).hide();
				$("#SCREEN-INCALL").show();
